    ...
    >
      <FormControl variant="standard">
        <Box sx={{ display: "flex", alignItems: "center" }}>
          <TooltipButton
            onClick={increment(-largeIncrement)}
            tooltipTitle={`-${largeIncrement}`}
            sx={{ minWidth: "30px", padding: "6px" }}
          >
            <KeyboardDoubleArrowDownIcon />
          </TooltipButton>
          <TooltipButton
            onClick={increment(-smallIncrement)}
            tooltipTitle={`-${smallIncrement}`}
            sx={{ minWidth: "30px", padding: "6px" }}
          >
...
function ArmManualInput({ commands, setCommands }) {
  ...
  useEffect(() => {
    setCommands((commands) => {
      return JSON.parse(
        JSON.stringify({ ...commands, arm: { ...controlValues } })
      );
    });
...
function DriveManualInput({ commands, setCommands }) {
  ...
  useEffect(() => {
    setCommands((commands) => {
      return JSON.parse(
        JSON.stringify({ ...commands, drive: { ...driveParams } })
      );
    });
...
function ScienceManualInput({ commands, setCommands }) {
  ...
  useEffect(() => {
    setCommands((commands) => {
      return JSON.parse(
        JSON.stringify({ ...commands, science: { ...scienceValues } })
      );
    });
  ...
  return (
    ...
    >
      <Button
        value={scienceValues.play}
        variant="contained"
        color={scienceValues.play ? "warning" : "primary"}
        size="large"
      ...
      <Button
        value={scienceValues.eStop}
        variant="contained"
        color={scienceValues.eStop ? "error" : "primary"}
        style={{ width: 500, height: 300 }}
        onClick={handleToggle("eStop")}
      >
